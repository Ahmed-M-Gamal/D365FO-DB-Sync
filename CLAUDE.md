...
- **Rerun safety**: for normal tables, the "missing" diff is computed from SQLDICTIONARY, which this tool never writes to — so a column a previous run already added physically would still look "missing" on every later Discover Tables, and `ALTER TABLE ADD` on an already-existing column fails ("column already exists"). Before attempting to add anything, `SyncMissingColumnsAsync` checks AxDB's actual `INFORMATION_SCHEMA.COLUMNS` (`AxDbDataService.GetTablesColumnsAsync`) and folds any column already physically present straight into the returned "added" set instead of re-issuing `ALTER TABLE` for it.
...
**AxDB SQLDICTIONARY Fallback (Physical Columns)**
- A table can exist physically in local AxDB without having a row in AxDB's own `SQLDICTIONARY` (the same metadata-vs-physical-schema gap documented under [Sequence Management](#sequence-management) for missing sequences — typically an incomplete/interrupted local Database Sync). Requiring a SQLDICTIONARY row on the AxDB side would otherwise skip every one of these tables with "not found in AxDB SQLDICTIONARY", even though there's a real table to copy into
- `PrepareTableListAsync` (`CopyOrchestrator.cs`) defers any table not found in `axDbCache` (rather than skipping it immediately) into a `pendingPhysicalCheck` list, then after the main loop does one batched `AxDbDataService.GetTablesColumnsAsync` (`INFORMATION_SCHEMA.COLUMNS`) call for all of them. A table found there is added to the run using its **physical** column list as `AxDbFields` (instead of SQLDICTIONARY's), with `TableInfo.AxDbTableId = 0` as a sentinel. Only a table found in neither AxDB SQLDICTIONARY nor as a physical table is actually skipped (`skippedNotInAxDbDict`)
- **Parameter limit**: SQL Server rejects a request with more than 2100 parameters, and `BuildInClause` (`Tier2DataService.cs`) uses one `@pN` parameter per table name in its `IN (...)` clause — a real risk here since a local environment with a large metadata gap can push `pendingPhysicalCheck` (and the analogous Sync UAT Schema lookups) into the thousands. `GetSystemTablesInfoAsync`, `GetTablesColumnsAsync` and `GetColumnDefinitionsAsync` all split their table-name list into ≤2000-name chunks (`ChunkNames`) and issue one query per chunk over the same open connection, merging results into one dictionary — keep any new batched-by-table-name query on this same chunking pattern
- **`AxDbTableId == 0` consequence**: `SEQ_{TableId}` naming has no reliable TableId without a SQLDICTIONARY row, so `UpdateSequenceAsync` (both copies) and `SqlPreviewBuilder.AppendSequenceUpdate` skip/note sequence management entirely for these tables instead of guessing a TableId — logged clearly so it's visible, not silent. Everything else (fetch, compare, insert) works normally since it only depends on `TableName` and the physical column list
- Discover Tables logs a one-line count of how many tables were recovered this way, plus a full skip-reason breakdown (excluded by pattern / zero rows / not in Tier2 dict / not in AxDB dict-or-physically / no copyable fields) whenever anything was skipped — added so a large "N skipped" total is diagnosable instead of one opaque number
...
- **Failure isolation**: a table failing (`FetchError`/`InsertError`) never stops the run — every other queued table is still processed by `ProcessTablesAsync`/`RetryFailedAsync`. Each table's own try/catch (`ProcessTableOptimizedAsync`/`ProcessTableStandardModeAsync`/`ProcessTableSystemModeAsync`) already isolates its own errors onto that table without rethrowing; the shared `CancellationTokenSource` is only ever cancelled by the user's **Stop** button, never automatically on a per-table failure. At the end of both methods, `LogFailedTablesReport` logs every table left in an error status with its name and error message, so failures are never just a count
- **Retry Failed** (`RetryFailedAsync`) only re-queues tables currently in `FetchError`/`InsertError` — it never re-touches an already-`Inserted` table
